...
wss.on('connection', async (ws, req) => {
  ...
  if (!rooms.has(snippetId)) {
    // Try specialized load from DB for persistence
    let initialContent = '';
    let expiresAt = null;
    try {
      const existing = await Snippet.findOne({ id: snippetId });
      if (existing) {
      ...
      if (data.type === 'op') {
        ...
        room.participants.forEach(client => {
          if (client !== ws && client.readyState === 1) {
            client.send(JSON.stringify({ type: 'op', content: data.content, from: socketId }));
          }
        });
      }
